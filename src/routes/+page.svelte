...
<main class="min-h-screen">
	{#if ready}
		...
		<section transition:fly={{ y: 50, duration: 400, delay: 1000 }} class="py-5">
			<a
				href="/projects"
				class="text-3xl font-light py-2 group flex gap-1 items-center border-b-2 border-light/10"
			>
				<p>Projects</p>
				<img
		...
		<section id="writing" transition:fly={{ y: 50, duration: 400, delay: 400 }} class="py-5">
			...
			{#each allPostInfo as post, index}
				<a
					class="flex justify-between items-center w-full border-b py-2 opacity-70 md:opacity-50 md:hover:opacity-100 transition-all"
					href={`/blog/${post.slug}/`}
				>
